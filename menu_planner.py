...
# 모델 상수(DEFAULT_MODEL / MODEL_CHOICES / MODEL_FALLBACKS)는 이 파일 상단에서만
# 정의한다. 아래 list_available_models() 가 실제 선택지를 API 에서 받아오고,
# 조회에 실패했을 때만 MODEL_FALLBACKS 로 되돌아간다.


def list_available_models(api_key: str | None = None) -> list[str]:
    """generateContent 를 지원하는 Gemini 모델 ID 목록을 API 에서 가져온다.

    키가 없거나 조회 실패 시 MODEL_FALLBACKS 를 반환한다(생성 중단 방지).
    """
    ...
    try:
        ...
        for m in client.models.list():
            ...
            out.append(short)
        out = sorted(set(out))
        return out or list(MODEL_FALLBACKS)
    except Exception:
        return list(MODEL_FALLBACKS)
...
def generate_validated_menu(
    ...
    for attempt in range(1, max_retries + 2):  # 최초 1회 + 재시도 max_retries회
        ...
        text = generate_menu(
            ...
            paused=paused, history_block=history_block, progress=progress,
        )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    parser.add_argument("--model", default=DEFAULT_MODEL,
                        help=f"사용할 Gemini 모델 ID (기본: {DEFAULT_MODEL}). "
