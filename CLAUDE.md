...
- `tests/test_exporters.py` 는 서식 수치(메뉴 열 너비 `MENU_COL_WIDTH`, 내용 기반
  행 높이, `wrap_text=True`, 인쇄 폭맞춤)를 잠근다.
  **이 테스트가 깨지면 인쇄물이 깨진 것**이다.
...
- 수치는 `exporters.py` 상단 상수에 모아 뒀다. **열 너비를 줄이면 메뉴 글자가 잘린다**
  (배달 A~G / 주방 B~H 모두 `MENU_COL_WIDTH`=26.0). 넓힌 열이 종이 폭을 넘어도
  `_page()` 의 `fitToWidth=1` 이 인쇄를 1페이지 폭에 맞춘다.
...
- 메뉴 행 높이는 칸에 실제로 들어간 글자를 보고 계산한다(`_cell_lines` /
  `calc_row_height`). 열 너비·폰트로 줄 수를 추정해 `_MIN_ROW_PT`~`_MAX_ROW_PT`
  사이로 자른다. 머리글 행만 `DLV_ROW_H_HEAD` / `KIT_ROW_H_HEAD` 고정.
...
- Gemini 모델: `menu_planner.DEFAULT_MODEL`(현재 `gemini-3.6-flash`) 한 곳에서만 정의.
  ...
  GUI/CLI가 고를 수 있는 목록은 `list_available_models()` 가 API 에서 받아오고,
  조회 전·실패 시에만 `MODEL_CHOICES`/`MODEL_FALLBACKS` 를 쓴다.
